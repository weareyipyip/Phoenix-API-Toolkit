defmodule PhoenixApiToolkit.TestHelpers do
  ...
  """
  @spec test_hmac_secret :: binary
  def test_hmac_secret(), do: "supersecretkey"
  ...
  @doc """
  Returns the default JWT payload generated by `gen_jwt/1`. The "exp" claim is set to
  NOW + 5 minutes.
  ...
  defp default_jws(), do: %{"alg" => default_jwk()["alg"], "kid" => default_jwk()["kid"]}

  @typedoc "Options for use by `gen_jwt/1`"
  @type gen_jwt_opts :: [
          jwk: map(),
          payload: map(),
          jws: map()
  ...
  ## Examples
      ...
      12345
  """
  @spec gen_jwt(gen_jwt_opts) :: binary
  def gen_jwt(overrides \\ []) do
    overrides =
      overrides
      |> Keyword.put_new_lazy(:jwk, &gen_jwk/0)
      |> Keyword.put_new_lazy(:payload, &gen_payload/0)
      |> Keyword.put_new_lazy(:jws, &gen_jws/0)
  ...
  def put_hmac(conn, body, secret \\ test_hmac_secret()) do
    conn
    |> Conn.put_req_header(
      "authorization",
